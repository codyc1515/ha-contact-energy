...
class ContactEnergyApi:
    ...
    def login(self):
        ...
        loginResult = requests.post(self._url_base + "/login/v2", json=data, headers=headers)
        if loginResult.status_code == requests.codes.ok:
            ...
            _LOGGER.error('Failed to login - check the username and password are valid', loginResult.text)
            return False
        return result
    ...
    def refresh_session(self):
        ...
        loginResult = requests.post(self._url_base + "/login/v2/refresh", json=data, headers=headers)
        if loginResult.status_code == requests.codes.ok:
            jsonResult = loginResult.json()
            ...
            _LOGGER.error('Failed to refresh session - check the username and password are valid', loginResult.text)
            return False
        return result
    ...
    def get_accounts(self):
        ...
        result = requests.get(self._url_base + "/customer/v2?fetchAccounts=true", headers=headers)
    ...
    def get_usage(self, year, month, day):
        ...
        response = requests.post(self._url_base + "/usage/v2/" + self._contractId + "?ba=" + self._accountId + "&interval=hourly&from=" + year + "-" + (month.zfill(2)) + "-" + (day.zfill(2)) + "&to=" + year + "-" + (month.zfill(2)) + "-" + (day.zfill(2)), headers=headers)
        data = {}
